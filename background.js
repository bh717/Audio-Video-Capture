...
const audioCapture = (timeLimit, muteTab, format, quality, limitRemoved) => {
  chrome.tabCapture.capture({audio: true}, (stream) => { // sets up stream for capture
    ...
    // alert(123);
    chrome.tabs.query({active:true, currentWindow: true}, (tabs) => startTabId = tabs[0].id) //saves start ta
    ...
    mediaRecorder.onComplete = (recorder, blob) => {
      ...
      // alert(audioURL);
      localStorage.setItem("URL", audioURL);
      recordedBlob = blob;
      chrome.runtime.sendMessage({type: "encodingComplete", format: format, audioURL, recordedBlob});
      mediaRecorder = null;
    }
...
const startCapture = function() {
  ...
  // alert(1);
  chrome.tabs.query({active: true, currentWindow: true}, (tabs) => {
   // sessionStorage.removeItem(tabs[0].id);
